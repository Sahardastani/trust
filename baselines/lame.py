...
######## add this in the forward and adapt instead of what we have:
            # outputs = model(x)
            # unary = -torch.log(outputs + 1e-10)
            # kernel = linear_affinity()(x.view(x.shape[0], -1))
            # logits = laplacian_optimization(unary.type(torch.float32), kernel.type(torch.float32)) 
            # return logits, None
