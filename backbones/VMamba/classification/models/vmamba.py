...
def get_index_from_experiment(experiment_name):
    # Extract numeric sequence from the experiment name
    match = re.search(r'(\d+)$', experiment_name)  # Extract numbers at the end
    if match:
        num_sequence = match.group(1)  # Get the matched numbers
        index = torch.tensor([int(digit) for digit in num_sequence])  # Reverse order
        return index
    else:
        raise ValueError("No numeric sequence found in experiment name.")
# =====================================================
...
class SS2Dv2:
    def __initv2__(
        ...
        **kwargs,    
        ...
        self.dataset_corrrupt = dataset_corrrupt
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        if _scan_mode == -1:
            ...
            if (self.experiment_name.startswith("test_traverses_")):
                index = get_index_from_experiment(self.experiment_name)
                xs = xs[:, index, :, :]

            elif (self.experiment_name == 'traverse_permutation'):
                index = get_index_from_experiment(s_var)
                xs = xs[:, index, :, :]
            ...
            if (self.experiment_name.startswith("test_traverses_") or self.experiment_name == 'traverse_permutation'):
                ys = ys[:, index, :, :, :]
    ...
    def forwardv2(self, x: torch.Tensor, s_: str = None, **kwargs):
        x = self.in_proj(x)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        kwargs.update(
            ...
            dataset_corrrupt=dataset_corrrupt,  
        )
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if self.dataset_corrrupt in {'cifar10c', 'cifar100c', 'visdac', 'pacs'}:
            self.classifier = nn.Sequential(OrderedDict(
                norm=norm_layer(self.num_features), # B,H,W,C
                permute=(Permute(0, 3, 1, 2) if not self.channel_first else nn.Identity()),
                avgpool=nn.AdaptiveAvgPool2d(1),
                flatten=nn.Flatten(1),
                head_=nn.Linear(self.num_features, num_classes),
            ))
        else:
            self.classifier = nn.Sequential(OrderedDict(
                norm=norm_layer(self.num_features), # B,H,W,C
    ...
    def forward(self, x: torch.Tensor, exp: str = None):

        if (self.experiment_name == 'traverse_permutation'):
            global s_var
            s_var = exp
...
def vmamba_tiny_s1l8(channel_first=True, experiment_name='cw', dataset='imagenetc'):
    if dataset in {'imagenetc', 'imagenetsketch', 'imagenetv2', 'imageneta', 'imagenetr'}:
        num_classes = 1000
    elif dataset in {'cifar10c'}:
        num_classes = 10
    elif dataset in {'cifar100c'}:
        num_classes = 100
    elif dataset in {'visdac'}:
        num_classes = 12
    ...
    return VSSM(
        ...
        patch_norm=True, norm_layer=("BN"), 
        downsample_version="v3", patchembed_version="v2", 
        use_checkpoint=False, posembed=False, imgsize=224, 
        experiment_name=experiment_name, dataset_corrrupt=dataset,
    )
