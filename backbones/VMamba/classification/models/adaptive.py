...
class SS2Dv2:
    def __initv2__(
        ...
        **kwargs,    
        ...
        if (self.experiment_name == 'adaptive_gate'):
            self.W_ = nn.Parameter(torch.ones(1, 1, 1))
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        if _scan_mode == -1:
            ...
            if (self.experiment_name == 'cw_exF'):
                with torch.no_grad():

                    distances = torch.cdist(x.view(B, D, -1).permute(0, 2, 1), x.view(B, D, -1).permute(0, 2, 1), p=2) 
                    distances = torch.exp(-distances ** 2 / (2 * torch.mean(distances) ** 2))
                    distances = torch.topk(distances, k=10, dim=-1, largest=False)[0]
                    mean_topk_values = distances.mean(dim=2, keepdim=True).permute(0, 2, 1)
                    mean_topk_values = cross_scan_fn(mean_topk_values.view(B, 1, H, W), in_channel_first=True, out_channel_first=True, scans=_scan_mode, force_torch=scan_force_torch)

                Cs += mean_topk_values

            elif (self.experiment_name == 'x_weight_exF'):
                with torch.no_grad():
                    distances = torch.cdist(x.view(B, D, -1).permute(0, 2, 1), x.view(B, D, -1).permute(0, 2, 1), p=2) 
                    distances = torch.exp(-distances ** 2 / (2 * torch.mean(distances) ** 2))
                    distances, indexes = torch.topk(distances, k=10, dim=-1, largest=False)

                    indexes = indexes.unsqueeze(1).expand(-1, D, -1, -1)
                    x_new = torch.gather(x.view(B, D, -1).unsqueeze(-1).expand(-1, -1, -1, 10), dim=2, index=indexes)
                    
                x_new = x_new * distances.unsqueeze(1)
                x = x + x_new.mean(dim=-1, keepdim=False).view(B, D, H, W)
                xs = cross_scan_fn(x, in_channel_first=True, out_channel_first=True, scans=_scan_mode, force_torch=scan_force_torch).view(B, -1, L)

            elif (self.experiment_name == 'cw_weight_exF'):
                knn = 10

                with torch.no_grad():
                    distances = torch.cdist(x.view(B, D, -1).permute(0, 2, 1), x.view(B, D, -1).permute(0, 2, 1), p=2) 
                    distances = torch.exp(-distances ** 2 / (2 * torch.mean(distances) ** 2))
                    distances, indexes = torch.topk(distances, k=knn, dim=-1, largest=False)

                    indexes = indexes.unsqueeze(1).expand(-1, D, -1, -1)
                    x_new = torch.gather(x.view(B, D, -1).unsqueeze(-1).expand(-1, -1, -1, knn), dim=2, index=indexes)
                    
                x_new = x_new * distances.unsqueeze(1)
                x_new = x_new.mean(dim=-1, keepdim=False).mean(dim=1, keepdim=True)
                Cs += cross_scan_fn(x_new.view(B, 1, H, W), in_channel_first=True, out_channel_first=True, scans=_scan_mode, force_torch=scan_force_torch)

            elif (self.experiment_name == 'cw_weight_random'):
                knn = 10
                ...
                with torch.no_grad():
                    ...
                    indexes = torch.stack([torch.randperm(num_points)[:knn] for _ in range(batch_size)])
                    distances = torch.gather(distances, dim=1, index=random_indices)
            ...
            elif (self.experiment_name == 'cw_weight_exF_softmax'):
                knn = 10
                with torch.no_grad():
                    distances = torch.cdist(x.view(B, D, -1).permute(0, 2, 1), x.view(B, D, -1).permute(0, 2, 1), p=2) 
                    distances = torch.exp(-distances ** 2 / (2 * torch.mean(distances) ** 2))
                ...
                distances = F.softmax(distances, dim=2)
                x_new = x_new * distances.unsqueeze(1)
                x_new = x_new.mean(dim=-1, keepdim=False).mean(dim=1, keepdim=True)
                Cs += cross_scan_fn(x_new.view(B, 1, H, W), in_channel_first=True, out_channel_first=True, scans=_scan_mode, force_torch=scan_force_torch)

            elif (self.experiment_name == 'cw_weight_similarity_False'):
                knn = 44
                with torch.no_grad():
                    x_norm = F.normalize(x.view(B, D, -1).permute(0, 2, 1), p=2, dim=-1)  # Normalize along feature dimension
                    similarity = torch.bmm(x_norm, x_norm.transpose(1, 2))
                    similarity = (similarity + 1) / 2

                    similarity, indexes = torch.topk(similarity, k=knn, dim=-1, largest=False)
                    indexes = indexes.unsqueeze(1).expand(-1, D, -1, -1)
                    x_new = torch.gather(x.view(B, D, -1).unsqueeze(-1).expand(-1, -1, -1, knn), dim=2, index=indexes)
                    
                x_new = x_new * similarity.unsqueeze(1)
                x_new = x_new.mean(dim=-1, keepdim=False).mean(dim=1, keepdim=True)
                Cs += cross_scan_fn(x_new.view(B, 1, H, W), in_channel_first=True, out_channel_first=True, scans=_scan_mode, force_torch=scan_force_torch)

            elif (self.experiment_name == 'cw_weight_similarity_True'):
                knn = 10
                with torch.no_grad():
                    x_norm = F.normalize(x.view(B, D, -1).permute(0, 2, 1), p=2, dim=-1)  # Normalize along feature dimension
                    similarity = torch.bmm(x_norm, x_norm.transpose(1, 2))
                    ...
                    similarity, indexes = torch.topk(similarity, k=knn, dim=-1, largest=True)
                    indexes = indexes.unsqueeze(1).expand(-1, D, -1, -1)
                    x_new = torch.gather(x.view(B, D, -1).unsqueeze(-1).expand(-1, -1, -1, knn), dim=2, index=indexes)
            ...
            elif (self.experiment_name == 'cw_weight_similarity_False_softmax'):
                knn = 10
                with torch.no_grad():
                    x_norm = F.normalize(x.view(B, D, -1).permute(0, 2, 1), p=2, dim=-1)  # Normalize along feature dimension
                    similarity = torch.bmm(x_norm, x_norm.transpose(1, 2))
            ...
            elif (self.experiment_name == 'cw_weight_similarity_True_softmax'):
                knn = 10
                with torch.no_grad():
                    x_norm = F.normalize(x.view(B, D, -1).permute(0, 2, 1), p=2, dim=-1)  # Normalize along feature dimension
                    similarity = torch.bmm(x_norm, x_norm.transpose(1, 2))
            ...
            elif (self.experiment_name == 'adaptive_gate'):
                if not (self.d == 0):
                    Cs = Cs * G_
            ...
            if (self.experiment_name == 'adaptive_gate'):
                ...
                ys_prime = ys.view(B, K, D, -1)
                ys_list = [
                    ys_prime[:, 0],
                    ys_prime[:, 1].reshape(B, -1, W, H).transpose(2, 3).reshape(B, D, -1),
                    ys_prime[:, 2].flip(-1),
                    ys_prime[:, 3].flip(-1).reshape(B, -1, W, H).transpose(2, 3).reshape(B, D, -1)
                ]
                ys_prime = torch.stack(ys_list, dim=1)

                # P_ = F.softmax(ys_prime, dim=1)
                # U_ = torch.mean((-1 * torch.sum(P_ * torch.log(P_ + 1e-6), dim=1)), dim=1, keepdim=True)

                U_ = torch.mean(torch.var(ys_prime, dim=1, unbiased=False), dim=1).unsqueeze(1)
                G_ = torch.sigmoid(self.W_ * U_)
                G_ = cross_scan_fn(G_.view(B, 1, H, W), in_channel_first=True, out_channel_first=True, scans=_scan_mode, force_torch=scan_force_torch)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        kwargs.update(
            ...
            initialize=initialize, forward_type=forward_type, channel_first=channel_first, experiment_name=experiment_name,
        )
    ...
    def _forward(self, input: torch.Tensor):
        ...
        if (self.experiment_name == 'adaptive_gate'):
            if (self.d == 0):
                ...
                x, G_ = input[0], input[1]
                if self.ssm_branch:
                    if self.post_norm:
                        x = x + self.drop_path(self.norm(self.op(x)))
                    else:
                        out, G_ = self.op((self.norm(x), G_))
                        x = x + self.drop_path(out)
                if self.mlp_branch:
                    if self.post_norm:
                        x = x + self.drop_path(self.norm2(self.mlp(x))) # FFN
            ...
            if (self.i_layer, self.d) in {(0, 1), (1, 1), (2, 7), (3, 1)}:
                return x
            else:
                return x, G_
        else:
...
class VSSM(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if (self.experiment_name == 'lame'):
            ...
            w = torch.cdist(gap(x).view(128, 768), gap(x).view(128, 768), p=2)
            w = torch.exp(-w ** 2 / (2 * torch.mean(w) ** 2)) #[128,128]

            x = self.classifier(x)

            Z_tilda = F.softmax(x, dim=1)
            regularizer = torch.sum(w * (Z_tilda @ Z_tilda.T))
            x = [x, regularizer]

            return x

        elif (self.experiment_name == 'msd'):
            x = self.patch_embed(x)
            if self.pos_embed is not None:
                pos_embed = self.pos_embed.permute(0, 2, 3, 1) if not self.channel_first else self.pos_embed
                x = x + pos_embed
            ...
            spatial_avg = x.mean(dim=(2, 3), keepdim=True)
            distance = x - spatial_avg
            mean_distance = torch.mean(distance, dim=1, keepdim=True)
            sigmoid = torch.sigmoid(mean_distance)
            x = x * sigmoid
...
# =====================================================
def vmamba_tiny_s1l8(channel_first=True, experiment_name='cw'):
    return VSSM(
